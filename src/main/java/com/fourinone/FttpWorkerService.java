...
public class FttpWorkerService extends WorkerService
{
	...
	@Delegate(interfaceName="com.fourinone.FttpWorker",methodName="gZipFile",policy=DelegatePolicy.Implements)
	public boolean gZip(String fp) throws RemoteException,FttpException{
		boolean r=false;
		if(fp == null || fp.length() ==0)
			return r;
		
		try{
			File file = new File(fp+GZipUtils.EXT);
			if(file.exists()){
				System.out.println("文件:"+fp+",已压缩");
			}else{
				System.out.println("文件:"+fp+",开始压缩");
			   GZipUtils.compress(fp, false);
			}
			r= true;
		}catch(Exception e){
			throw new FttpException(e);
