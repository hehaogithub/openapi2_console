...
public class SystemInitListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent sce) {
		...
		try {
			scheduler = JobTools.getScheduler();
			scheduler.startDelayed(5);
			// scheduler.start();
			log.info("job调度器" + 5 + "s后启动...");
		} catch (SchedulerException e) {
			// TODO Auto-generated catch block
			log.error("启动job调度器失败..", e);
		}
	...
	private void systemStartup(ServletContext servletContext) {
		...
		if (success) {
			...
			log.info("开始加载字典...");
			Reader g4Reader = (Reader) SpringBeanLoader
		...
		if (dbString.indexOf("ora") > -1) {
			...
			if (log.isErrorEnabled()) {
				log.error(G4Constants.Exception_Head
						+ "G4Studio平台目前还不支持你使用的数据库产品.如需获得支持,请和我们联系!");
			}
			System.exit(0);
		}
	}
